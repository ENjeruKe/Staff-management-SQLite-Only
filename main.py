#
# This script saves employee data to SQLite only.
#--------------------------------------------------------
...
sqlClient = mySqlClient(database='employees.db')
...
window.title("Staff Management System")
...
def homeScreen():
    ...
    frame = Frame(window)
    frame.pack()
    optionsFrame = LabelFrame(frame, text="Options")
    optionsFrame.grid(row=0, column=0, padx=20, pady=10)
    addButton = Button(optionsFrame, text='Add New Employee', height=2, width=15, padx=20, pady=20, font='lucida 10 normal', command=addScreen)
    addButton.pack(padx=10, pady=10)
    deleteButton = Button(optionsFrame, text='Delete An Employee', height=2, width=15, padx=20, pady=20, font='lucida 10 normal', command=deleteScreen)
    deleteButton.pack(padx=10, pady=10)
    updateButton = Button(optionsFrame, text='Update An Employee', height=2, width=15, padx=20, pady=20, font='lucida 10 normal', command=updateScreen)
    updateButton.pack(padx=10, pady=10)
    viewButton = Button(optionsFrame, text='View All Employees', height=2, width=15, padx=20, pady=20, font='lucida 10 normal', command=allEmployeeScreen)
...
def addScreen():
    ...
    def enterData():
        ...
        if firstname and lastname and birthDate and salary and joiningDate and department:
            ...
            messagebox.showinfo(title="Success", message="A new employee details added into the database.")
...
def deleteScreen():
    ...
    def nextDelete():
            ...
            def delete():
                ...
                if len(selectedEmployee) == 0:
                    ...
                    for item in selectedEmployee:
                        ...
                        deleteScreen()
                        messagebox.showinfo(title="Success", message="Deleted employee details.")
        ...
        if usingOptValue == 'Find Employee Using':
            ...
            valueLabel = Label(nextDeleteFrame, text=f"Enter {usingOptValue}")
                ...
                if len(selectedEmployee) == 0:
                    ...
                    def enterData():
                        ...
                        if firstname and lastname and birthDate and salary and joiningDate and department:
                            ...
                            messagebox.showinfo(title="Success", message="Updated Employee Details")
                            updateScreen()
                        else: 
                            messagebox.showwarning(title="Error", message="All fields are required.")
...
def allEmployeeScreen():
    ...
    if len(employees) == 0:
        messagebox.showwarning(title="Error", message="No employees in the database.")
    else:
        for widget in window.winfo_children():
            widget.destroy()
        frame = Frame(window)
...
homeScreen()
window.mainloop()
#[END OF main_SQLite_Version_20241004.py]
